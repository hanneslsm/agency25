 ...
 */
function agency25_enqueue_editor_styles() {
	...
	wp_enqueue_style(
		'agency25-editor-style',
		$editor_style_path,
		$editor_style_asset['dependencies'],
		$editor_style_asset['version']
	);
}
add_action( 'enqueue_block_editor_assets', 'agency25_enqueue_editor_styles' );
	...
	if ( $block_styles ) {
		foreach ( $block_styles as $style_path ) {
			...
			wp_enqueue_block_style(
				$block_name,
				array(
					'handle' => 'agency25-' . $filename . '-style',
					'src'    => $style_uri,
				)
			);
		}
...
add_action( 'init', 'agency25_enqueue_block_styles' );
